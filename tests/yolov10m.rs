...
mod common;
#[tokio::test]
async fn test_yolov10m_inference() -> Result<(), Box<dyn Error>> {
    // 使用 CPU 设备进行测试
    let device = Device::cuda_if_available(0)?;
    let filename = download_if_not_exists("yolov10m.safetensors").await?;

    println!("loading model weights: {filename:?}");
    // 加载 yolov10m 模型权重
    let vb = unsafe {
        VarBuilder::from_mmaped_safetensors(&["yolov10m.safetensors"], DType::F32, &device)
    }?;
    ...
    img.save_with_format(
        "./target/yolov10m_test_result.jpg",
