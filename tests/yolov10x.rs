...
mod common;
#[tokio::test]
async fn test_yolov10x_inference() -> Result<(), Box<dyn Error>> {
    // 使用 CPU 设备进行测试
    let device = Device::cuda_if_available(0)?;
    let filename = download_if_not_exists("yolov10x.safetensors").await?;
    ...
    // 创建 YOLOv10x 模型实例
    let yolo = YoloV10::load(vb, Multiples::x(), 80)?;

    // 执行推理
    let output = yolo.forward(&image_t)?;

    // 验证输出形状是否正确
    // YOLOv10x 输出形状为 [1, 300, 6]，与 l/b 模型相同
    ...
    img.save_with_format(
        "./target/yolov10x_test_result.jpg",
        image::ImageFormat::Jpeg,
    )
    .unwrap();

    println!("YOLOv10x inference test passed!");
